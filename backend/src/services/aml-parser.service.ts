 ...
 */
export async function extractCubeFromAml(
  amlPath: string,
  cubeOutputPath: string
): Promise<void> {
  const fileBuffer = await readFile(amlPath);
  ...
  if (isBinaryKola) {
    ...
    cubeLines = [
      `# Extracted from ARRI Look File 2 Binary (KOLA)`,
    ...
    // Adobe cube requirements: Red changes fastest, Blue slowest
    // ARRI KOLA binary layout: Blue changes fastest, Red slowest (stored as RGB)
    for (let bi = 0; bi < lutSize; bi++) {
      for (let gi = 0; gi < lutSize; gi++) {
        for (let ri = 0; ri < lutSize; ri++) {
          // Calculate the correct offset in the binary AML byte array
          const amlIndex = bi + (gi * lutSize) + (ri * lutSize * lutSize);
          const byteOffset = amlIndex * 6; // 3 channels * 2 bytes each

          const r = (lutData.readUInt16LE(byteOffset) / 65535.0).toFixed(6);
          const g = (lutData.readUInt16LE(byteOffset + 2) / 65535.0).toFixed(6);
          const b = (lutData.readUInt16LE(byteOffset + 4) / 65535.0).toFixed(6);
    ...
    // Check if this is already a CUBE file (user might have uploaded .cube directly)
    if (amlContent.trimStart().startsWith("LUT_") || amlContent.trimStart().startsWith("#") || amlContent.trimStart().startsWith("TITLE")) {
      await writeFile(cubeOutputPath, amlContent, "utf-8");
      return;
    }
    ...
    if (floatData.length !== expectedEntries) {
      throw new Error(
        `LUT3D size ${lutSize} expects ${expectedEntries} floats, but got ${floatData.length}`
      );
    }

    cubeLines = [
      `# Extracted from ARRI Look File 2 XML (AML)`,
  ...
  await writeFile(cubeOutputPath, cubeLines.join("\n"), "utf-8");
}
