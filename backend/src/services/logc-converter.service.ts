...
function linearToLogC4(x: number): number {
  if (x >= LOGC4.t) {
    // Log section
 ...
 */
function inverseDrtToLogC4(displayValue: number): number {
 ...
 */
export async function convertToLogToLogCmt(
  inputPath: string,
  outputPath: string
): Promise<void> {
  ...
  if (dataValues.length !== expectedEntries) {
    throw new Error(
      `CUBE file has ${dataValues.length} entries but LUT_3D_SIZE ${lutSize} expects ${expectedEntries}`
    );
  }
  ...
  for (let bi = 0; bi < lutSize; bi++) {
    for (let gi = 0; gi < lutSize; gi++) {
      for (let ri = 0; ri < lutSize; ri++) {
        ...
        const [dispR, dispG, dispB] = trilinearInterpolate(
          lutData, lutSize, rLogC3, gLogC3, bLogC3
        );
...
function linearToLogC3(x: number): number {
  if (x > LOGC3.cut) {
...
function logC4ToLogC3(logC4Value: number): number {
 ...
 */
export async function convertCubeLogC3ToLogC4(
  inputPath: string,
  outputPath: string
): Promise<void> {
  const content = await readFile(inputPath, "utf-8");
  ...
  for (let bi = 0; bi < lutSize; bi++) {
    for (let gi = 0; gi < lutSize; gi++) {
      for (let ri = 0; ri < lutSize; ri++) {
        ...
        const [outR, outG, outB] = trilinearInterpolate(
          lutData, lutSize, rLogC3, gLogC3, bLogC3
        );
 ...
 */
export async function generateIdentityCmtCube(
  outputPath: string,
  size: number = 17
): Promise<void> {
  const header = [
 ...
 */
export async function generateInverseGamutCmtCube(
  outputPath: string,
  size: number = 33
): Promise<void> {
  const header = [
 ...
 */
export async function generateGamutConversionCmtCube(
  outputPath: string,
  size: number = 33
): Promise<void> {
  const header = [
 ...
 */
export async function generateDiagnosticCmtCube(
  outputPath: string,
  size: number = 17
): Promise<void> {
  const header = [
 ...
 */
export async function convertCubeAwg4LogC3ToAwg3LogC3(
  inputPath: string,
  outputPath: string
): Promise<void> {
  const content = await readFile(inputPath, "utf-8");
 ...
 */
export async function convertCubeAwg4LogC4ToAwg3LogC3(
  inputPath: string,
  outputPath: string
): Promise<void> {
  const content = await readFile(inputPath, "utf-8");
  ...
  for (let bi = 0; bi < lutSize; bi++) {
    for (let gi = 0; gi < lutSize; gi++) {
      for (let ri = 0; ri < lutSize; ri++) {
        ...
        // Step 2: Apply AWG4 → AWG3 gamut matrix
        const [rAwg3Linear, gAwg3Linear, bAwg3Linear] = applyMatrix(
          AWG4_TO_AWG3_MATRIX, rLinear, gLinear, bLinear
        );
